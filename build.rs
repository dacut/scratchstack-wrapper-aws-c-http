...
fn main() {
    ...
    let aws_c_compression_include_dir =
        PathBuf::from(var("DEP_AWS_C_COMPRESSION_INCLUDE").expect("DEP_AWS_C_COMPRESSION_INCLUDE not set"));
    let aws_c_compression_include_dir_str = aws_c_compression_include_dir.to_string_lossy();

    let src_include_dir = root.join("include");
    let dst_include_dir = out_dir.join("include");
    let src_lib_include_dir = src_include_dir.join("aws").join("http");
    let dst_lib_include_dir = dst_include_dir.join("aws").join("http");
    ...
    let mut builder = builder()
        ...
        .clang_arg(format!("-I{aws_c_compression_include_dir_str}"))
        .derive_debug(true)
        .derive_default(true)
    ...
    if n_includes == 0 {
        panic!("No header files found in include/aws/http");
    }
